...
if __name__ == '__main__':
    objdll = ctypes.windll.LoadLibrary('./msdk_x64.dll')
    objdll.M_Open.restype = ctypes.c_uint64
    hdl = objdll.M_Open(1)
    print("open handle = " + str(hdl))
    time.sleep(3)  #sleep 3s 加延时，延时期间将鼠标点到记事本里，方便调试用
    #键盘单击
    res = objdll.M_KeyPress(ctypes.c_uint64(hdl), 4, 2) #打开
    print("M_KeyPress =  " + str(res))
    #输入字符串
    VbufWr = create_string_buffer(128)      # 定义一个可变字符串变量，长度为128
    VbufWr.raw = b"12345678"
    print(" WrData = " + repr(VbufWr.raw))
    pWrBuf = c_char_p() #定义一个char型指针变量
    pWrBuf = pointer(VbufWr)
    res = objdll.M_KeyInputString(ctypes.c_uint64(hdl), pWrBuf, 8) #输入8个字符
    print("M_KeyInputString =  " + str(res) + " Data = " + repr(VbufWr.raw))
    #读取鼠标位置
    ...
    res = objdll.M_Close(ctypes.c_uint64(hdl))
